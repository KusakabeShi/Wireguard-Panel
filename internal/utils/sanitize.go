...
func ValidateIPorDomain(endpoint string) error {
	if endpoint == "" {
...
func IsSafeName(name string) error {
	...
	if strings.ContainsAny(name, "\r\n") {
		return fmt.Errorf("name contains new lines")
	}
...
var fwMarkRegex = regexp.MustCompile(`^(0x[0-9a-fA-F]+|[1-9][0-9]+)$`)

// IsValidFWMark validates a firewall mark value
// Accepts decimal (0-4294967295) or hex (0x0-0xFFFFFFFF) format
func IsValidFWMark(mark string) error {
	if mark == "" {
		return fmt.Errorf("firewall mark cannot be empty")
	}

	var err error

	// Whitelist: only allow decimal digits or hex format (0x followed by hex digits)
	if !fwMarkRegex.MatchString(mark) {
		return fmt.Errorf("firewall mark contains invalid characters: must be decimal digits or hex format (0x...)")
	}
	...
	if strings.HasPrefix(mark, "0x") {
		_, err = strconv.ParseUint(mark, 0, 32)
	} else {
		// Handle decimal format
		_, err = strconv.ParseUint(mark, 10, 32)
	}

	if err != nil {
		return fmt.Errorf("invalid firewall mark format: %v", err)
	}

	// Value is already validated to be within uint32 range by ParseUint(*, *, 32)
	// Range is 0 to 4294967295 (0xFFFFFFFF)
	return nil
}
